...
**  For command details see #HS_NOOP_CC, #HS_RESET_CC, #HS_ENABLE_APPMON_CC,
*#HS_DISABLE_APPMON_CC,
**  #HS_ENABLE_EVENTMON_CC, #HS_DISABLE_EVENTMON_CC, #HS_ENABLE_ALIVENESS_CC,
...
typedef struct {
    ...
    uint8 CurrentAlivenessState; /**< \hstlmmnemonic \HS_CPUALIVESTATE
                                           \brief Status of HS Aliveness
                                    Indicator     */
    uint8
        CurrentCPUHogState;      /**< \hstlmmnemonic \HS_CPUHOGSTATE
    ...
    uint32
        InvalidEventMonCount; /**< \hstlmmnemonic \HS_INVALIDEVTAPPCNT
                                        \brief Total count of Invalid Event
                                 Monitors Monitored by the Events Monitor   */
    uint32 AppMonEnables[((HS_MAX_MONITORED_APPS - 1) /
                          HS_BITS_PER_APPMON_ENABLE) +
                         1]; /**< \hstlmmnemonic \HS_APPSTATUS
\brief Enable states of App Monitor Entries */
    uint32 MsgActExec;       /**< \hstlmmnemonic \HS_MSGACTEXEC
                                       \brief Number of Software Bus Message Actions
                                Executed */
    ...
    uint32 ExeCounts[HS_MAX_EXEC_CNT_SLOTS]; /**< \hstlmmnemonic
                                                \HS_EXECUTIONCTR \brief
                                                Execution Counters */
#endif
