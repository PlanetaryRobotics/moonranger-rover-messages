...
**
**  \par Limits
**      All versions of the cFE currently support only CCSDS as the message
*format
**      Defining MESSAGE_FORMAT_IS_CCSDS_VER_2 implements the APID extended
*header format
...
/** \{ */
#define CFE_MISSION_SB_TIME_32_16_SUBS \
    1 /**< \brief 32 bits seconds + 16 bits subseconds (units = 2^^-16)  */
#define CFE_MISSION_SB_TIME_32_32_SUBS \
    2 /**< \brief 32 bits seconds + 32 bits subseconds (units = 2^^-32)  */
#define CFE_MISSION_SB_TIME_32_32_M_20 \
    3 /**< \brief 32 bits seconds + 20 bits microsecs + 12 bits reserved */
/** \} */
...
**      These will now differ in format when using CCSDS version 2 as they will
*no longer
...
**             numbers.
**
**             For MESSAGE_FORMAT_IS_CCSDS_VER_2 These base MsgIds values are
*dependent on the
**             values returned by the following SB Macros to form a 16 bit
*message ID (default
**             macro definitions are in cfe_sb_msg_id_utils.h, default values
*below are
...
**
**  \par Description:
**       Defines the size of the shell command tlm packet segments.The shell
*command
**       output size is dependant on the shell command itself. If the shell
*output
...
**
**  \par Description:
**       Dictates the maximum number of unique Pipes the SB message defintions
*will hold.
**
**       This affects the layout of command/telemetry messages but does not
*affect run
...
**       any possible neighboring fields without implicit padding.
*/
#define CFE_MISSION_ES_CDS_MAX_NAME_LEN \
