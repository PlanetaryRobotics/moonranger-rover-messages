...
**
**  Purpose:
**	           Unit specification for Event services command codes and data
*structures.
**
**  Design Notes:
**
...
**      an application's event type could result in flooding of the ground
*system.
**
...
**      command to take effect, applications must be registered for Event
*Service.
**
...
**       - The clearing of the Event Type Active Flag in The Event Type Active
*Flag in EVS App Data File
**
**  \par Error Conditions
**      This command may fail for the following reason(s):
**      - Invalid Event Type Selection
...
**       - The setting of the Active Flag in The Active Flag in EVS App Data
*File
**
**  \par Error Conditions
**      This command may fail for the following reason(s):
...
**
**  \par Description
**      This command sets the command specified application's event counter to
*zero.
**      Note: In order for this command to take effect, applications must be
*registered
**      for Event Service.
...
**      they react to changes in the counter value that is reset by this
*command.
**
...
**      ground system.
**
**  \sa #CFE_EVS_RESET_FILTER_CC, #CFE_EVS_RESET_ALL_FILTERS_CC,
*#CFE_EVS_ADD_EVENT_FILTER_CC, #CFE_EVS_DELETE_EVENT_FILTER_CC
*/
...
**       None.
**
**  \sa #CFE_EVS_SET_FILTER_CC, #CFE_EVS_RESET_ALL_FILTERS_CC,
*#CFE_EVS_ADD_EVENT_FILTER_CC, #CFE_EVS_DELETE_EVENT_FILTER_CC
*/
...
**       None.
**
**  \sa #CFE_EVS_SET_FILTER_CC, #CFE_EVS_RESET_FILTER_CC,
*#CFE_EVS_ADD_EVENT_FILTER_CC, #CFE_EVS_DELETE_EVENT_FILTER_CC
*/
...
**
**  \par Description
**      This command adds the given filter for the given application identifier
...
**
**  \sa #CFE_EVS_SET_FILTER_CC, #CFE_EVS_RESET_FILTER_CC,
*#CFE_EVS_RESET_ALL_FILTERS_CC, #CFE_EVS_DELETE_EVENT_FILTER_CC
*/
...
**
**  \par Description
**      This command removes the given filter for the given application
...
**
**  \sa #CFE_EVS_SET_FILTER_CC, #CFE_EVS_RESET_FILTER_CC,
*#CFE_EVS_RESET_ALL_FILTERS_CC, #CFE_EVS_ADD_EVENT_FILTER_CC
*/
...
**
**  \par Description
**       This command writes all application data to a file for all applications
*that
**           have registered with the EVS.  The application data includes the
*Application ID,
...
**
**  \par Criticality
**       Writing a file is not particularly hazardous, but if proper file
*management is not
**       taken, then the file system can fill up if this command is used
*repeatedly.
**
...
*repeatedly.
**
**  \sa #CFE_EVS_WRITE_APP_DATA_FILE_CC, #CFE_EVS_SET_LOG_MODE_CC,
 ...
 * Create a unique type for each one so the prototypes will follow the naming
 * pattern, allowing each command to evolve independently.
 */
...
**/
typedef struct {
    char LogFilename[CFE_MISSION_MAX_PATH_LEN]; /**< \brief Filename where log
...
**/
typedef struct {
    char AppDataFilename[CFE_MISSION_MAX_PATH_LEN]; /**< \brief Filename where
                                                       applicaton data is to be
                                                       written */
...
**
** For command details, see #CFE_EVS_SET_EVENT_FORMAT_MODE_CC and/or
...
**
** For command details, see #CFE_EVS_ENABLE_EVENT_TYPE_CC,
*#CFE_EVS_DISABLE_EVENT_TYPE_CC,
**                          #CFE_EVS_ENABLE_PORTS_CC and/or
...
**
** For command details, see #CFE_EVS_ENABLE_APP_EVENTS_CC,
*#CFE_EVS_DISABLE_APP_EVENTS_CC,
**                          #CFE_EVS_RESET_APP_COUNTER_CC and/or
*#CFE_EVS_RESET_ALL_FILTERS_CC
**
**/
typedef struct {
    char AppName[CFE_MISSION_MAX_API_LEN]; /**< \brief Application name to use
                                              in the command*/
...
**
** For command details, see #CFE_EVS_ENABLE_APP_EVENT_TYPE_CC and/or
...
/*
 * The CFE_EVS_AppNameEventIDMaskCmd_t is shared by several EVS command
 * handlers. Create a unique type for each one so the prototypes will follow the
 * naming pattern, allowing each command to evolve independently.
 */
...
typedef struct {
    ...
    uint16
        AppMessageSentCounter; /**< \cfetlmmnemonic \EVS_APPMSGSENTC
                                    \brief Application message sent counter */
    uint8
        AppEnableStatus; /**< \cfetlmmnemonic \EVS_APPENASTAT
...
typedef struct {
    ...
    uint8
        MessageFormatMode;     /**< \cfetlmmnemonic \EVS_MSGFMTMODE
    ...
    uint8 UnregisteredAppCounter; /**< \cfetlmmnemonic \EVS_UNREGAPPC
                                       \brief Unregistered application message
                                     send counter */
    ...
    CFE_EVS_AppTlmData_t
        AppData[CFE_MISSION_ES_MAX_APPLICATIONS]; /**< \cfetlmmnemonic \EVS_APP
                                               \brief Array of registered
...
typedef struct {
    ...
    char Message[CFE_MISSION_EVS_MAX_MESSAGE_LENGTH]; /**< \cfetlmmnemonic
                                                   \EVS_EVENT \brief Event
                                                   message string */
 ...
 * In some circumstances applications may subscribe to telemetry from this
 * component, and therefore the name changes may break existing code.  For these
 * situations a typedef is created to continue supporting the previous name.
 * These should be removed in the next CFE release.
 */
...
/* the CFE_EVS_HK_TLM_LNGTH macro is no longer used in EVS, but might be used in
 * apps */
