...
*
**      The MsgId is a mission defined message identifier to publish or
*subscribe to that must be
**      unique within the system(s). CFE_SB_MsgId_t is a uint32 that can be
*created from any
**      combination of bits from the primary header SID (StreamId) and the
*secondary header APID Qualifiers
**
**      Implementation is based on CCSDS Space Packet Protocol 133.0.B-1 with
*Technical Corrigendum 2, September 2012
**      Multi-mission Interoperable extended secondary headers should be
*registered in Space
**      Assigned Numbers Authority (SANA). The process for SANA registration is
*documented in
...
**
**
**     The APID qualifier System field can be populated in the Secondary header
*but
**     but will be ignored in the default case for SB/SBN routing purposes. It
*is suggested
**     that the CCSDS Spacecraft ID be used for that field.
**
** Notes: The following 4 terms have been or are used in the cFS architecture
*and implementation
**
**      StreamId - First 16 bits of CCSDS Space Packet Protocol (SPP)
*133.0-B.1c2 Blue Book
**                 packet primary header. It contains the 3 bit Version Number,
*1 bit Packet Type ID,
**                 1 bit Secondary Header flag, and 11 bit Application Process
*ID
**                 It was used in earlier cFS implementaions and defined here
*for historical reference
**                 It is NOT exposed to user applications.
**
**      MsgId    - Unique numeric message identifier within a mission namespace.
*It is used by cFS
**                 applications to the identify messages for publishing and
*subscribing
**                 It is used by the SB API and encoded in a mission defended
*way in the header of
**                 all cFS messages.
**                 It is exposed to all cFS applications
**
**      ApId     - CCSDS Application Process Id field in the primary header.
**                 It has default bit mask of 0x07FF and is part of the cFS
*message Id
**                 It should not be confused with the cFE Executive Services
*(ES) term appId which
**                 identifies the software application/component
**                 It is NOT exposed to user applications.
**
**      MsgIdkey - This is a unique numeric key within a mission namespace that
*is used with
**                 cFS software bus internal structures.
**                 It is algorithmically created in a mission defined way from
*the MsgId to support
...
CFE_SB_MsgId_t CFE_SB_GetMsgId(const CFE_SB_Msg_t *MsgPtr) {
    ...
    /*   The default is to init this field to the Spacecraft ID but ignore for
     * routing.   */
    /*   To fully implement this field would require significant SB optimization
     * to avoid */
...
void CFE_SB_SetMsgId(CFE_SB_MsgPtr_t MsgPtr, CFE_SB_MsgId_t MsgId) {
    ...
    CCSDS_WR_SUBSYSTEM_ID(MsgPtr->SpacePacket.ApidQ,
                          CFE_SB_RD_SUBSYS_ID_FROM_MSGID(MsgIdVal));
