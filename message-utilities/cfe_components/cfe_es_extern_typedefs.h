...
enum CFE_ES_SystemState
{
    ...
    /**
     * @brief startup apps have all completed their early init, but not
     * necessarily operational yet
     */
 ...
 *
 *
 * These values are used with the #CFE_ES_WaitForSystemState API call to
 * synchronize application startup.
 *
 * @note These are defined in order so that relational comparisons e.g. if
 * (STATEA < STATEB) are possible
 *
...
enum CFE_ES_AppState
{
    ...
    /**
     * @brief App thread has started, app performing early initialization of its
     * own data
     */
