...
#define CFE_MAKE_BIG32(n)                                     \
    ((((n) << 24) & 0xFF000000) | (((n) << 8) & 0x00FF0000) | \
 ...
 * -------*/
/* This is the verion of the data sheet that defines the packet payload format
 * and other */
...
typedef struct {
    ...
    CCSDS_APIDqualifiers_t ApidQ; /**< \brief CCSDS APID Qualifier Secondary
                                     Header #CCSDS_APIDqualifiers_t */
} CCSDS_APIDQHdr_t;

typedef struct {
    /*
     * In Version 1 mode, the standard / non-APID qualified header is used for
     * all packets
     */
    ...
    /*
     * In Version 2 mode, the extended / APID qualified header is used for all
     * packets
     */
    CCSDS_APIDqualifiers_t ApidQ; /**< \brief CCSDS APID Qualifier Secondary
                                     Header #CCSDS_APIDqualifiers_t */
...
typedef struct {
    CCSDS_SpacePacket_t
        SpacePacket; /**< \brief Standard Header on all packets  */
 ...
 * These types were defined by CFE 6.5 and below and applications may still use
 * them. These typdefs provide compatibility for existing code.  These should be
...
#define CCSDS_WR_BITS(word, mask, shift, value) \
    ((word) =                                   \
         (uint16)(((word) & ~mask) | (((value) & (mask >> shift)) << shift)))
...
#define CCSDS_WR_SID(phdr, value) \
    (((phdr).StreamId[0] = (value >> 8)), ((phdr).StreamId[1] = (value & 0xff)))
...
#define CCSDS_WR_APID(phdr, value)                                \
    ((((phdr).StreamId[0] =                                       \
           ((phdr).StreamId[0] & 0xF8) | ((value >> 8) & 0x07))), \
...
#define CCSDS_WR_SHDR(phdr, value) \
...
#define CCSDS_WR_TYPE(phdr, value) \
...
#define CCSDS_WR_SEQFLG(phdr, value) \
...
#define CCSDS_WR_FC(shdr, value) \
    CCSDS_WR_BITS((shdr).FunctionCode, 0x7F, 0, value)
...
#define CCSDS_RD_SUBSYSTEM_ID(shdr) \
    ((((shdr).APIDQSubsystem[0] & 0x01) << 8) + ((shdr).APIDQSubsystem[1]))
#define CCSDS_RD_SYSTEM_ID(shdr) \
    (((shdr).APIDQSystemId[0] << 8) + ((shdr).APIDQSystemId[1]))

#define CCSDS_WR_EDS_VER(shdr, val) \
    ((shdr).APIDQSubsystem[0] =     \
         ((shdr).APIDQSubsystem[0] & 0x07) | (((val)&0x1f) << 3))
#define CCSDS_WR_ENDIAN(shdr, val) \
    ((shdr).APIDQSubsystem[0] =    \
         ((shdr).APIDQSubsystem[0] & 0xFB) | (((val)&0x01) << 2))
#define CCSDS_WR_PLAYBACK(shdr, val) \
    ((shdr).APIDQSubsystem[0] =      \
         ((shdr).APIDQSubsystem[0] & 0xFD) | (((val)&0x01) << 1))

#define CCSDS_WR_SUBSYSTEM_ID(shdr, val)                              \
    (((shdr).APIDQSubsystem[0] =                                      \
          ((shdr).APIDQSubsystem[0] & 0xFE) | ((val & 0x0100) >> 8)), \
...
#define CCSDS_CLR_PRI_HDR(phdr)                                             \
    ((phdr).StreamId[0] = 0, (phdr).StreamId[1] = 0,                        \
     (phdr).Sequence[0] = (CCSDS_INIT_SEQFLG << 6), (phdr).Sequence[1] = 0, \
     (phdr).Length[0] = 0, (phdr).Length[1] = 0)

#define CCSDS_CLR_SEC_APIDQ(shdr)                                \
    ((shdr).APIDQSubsystem[0] = 0, (shdr).APIDQSubsystem[1] = 0, \
     (shdr).APIDQSystemId[0] = 0, (shdr).APIDQSystemId[1] = 0)

/* Clear command secondary header. */
#define CCSDS_CLR_CMDSEC_HDR(shdr) \
    ((shdr).FunctionCode = CCSDS_INIT_FC, (shdr).Checksum = CCSDS_INIT_CHECKSUM)

#define CCSDS_WR_SEC_HDR_SEC(shdr, value)  \
    shdr.Time[0] = ((value >> 24) & 0xFF), \
    shdr.Time[1] = ((value >> 16) & 0xFF), \
    shdr.Time[2] = ((value >> 8) & 0xFF), shdr.Time[3] = ((value)&0xFF)

#define CCSDS_RD_SEC_HDR_SEC(shdr)                                    \
    (((uint32)shdr.Time[0]) << 24) | (((uint32)shdr.Time[1]) << 16) | \
        (((uint32)shdr.Time[2]) << 8) | ((uint32)shdr.Time[3])

/* Clear telemetry secondary header. */
#if (CFE_MISSION_SB_PACKET_TIME_FORMAT == CFE_MISSION_SB_TIME_32_16_SUBS)
/* 32 bits seconds + 16 bits subseconds */
#define CCSDS_CLR_TLMSEC_HDR(shdr)                               \
    ((shdr).Time[0] = 0, (shdr).Time[1] = 0, (shdr).Time[2] = 0, \
     (shdr).Time[3] = 0, (shdr).Time[4] = 0, (shdr).Time[5] = 0)
...
#define CCSDS_RD_SEC_HDR_SUBSEC(shdr) \
    (((uint32)shdr.Time[4]) << 8) | ((uint32)shdr.Time[5])
#elif (                                                                      \
...
#define CCSDS_CLR_TLMSEC_HDR(shdr)                               \
    ((shdr).Time[0] = 0, (shdr).Time[1] = 0, (shdr).Time[2] = 0, \
     (shdr).Time[3] = 0, (shdr).Time[4] = 0, (shdr).Time[5] = 0, \
     (shdr).Time[6] = 0, (shdr).Time[7] = 0)

#define CCSDS_WR_SEC_HDR_SUBSEC(shdr, value) \
    shdr.Time[4] = ((value >> 24) & 0xFF),   \
    shdr.Time[5] = ((value >> 16) & 0xFF),   \
    shdr.Time[6] = ((value >> 8) & 0xFF), shdr.Time[7] = ((value)&0xFF)

#define CCSDS_RD_SEC_HDR_SUBSEC(shdr)                                 \
    (((uint32)shdr.Time[4]) << 24) | (((uint32)shdr.Time[5]) << 16) | \
        (((uint32)shdr.Time[6]) << 8) | ((uint32)shdr.Time[7])
#endif
